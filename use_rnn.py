...
def load_data():
    ...
    if VOCAB_FILE_FILENAME == "STDIN":
        f = sys.stdin
    else:
        f = open(VOCAB_FILE_FILENAME, 'r', encoding="utf-8")
...
for output_idx, from_hidden_to_idxth_output in enumerate(model.W_ho):
    for hidden_idx, weight in enumerate(from_hidden_to_idxth_output):
        ...
        print(f"weight HIDDEN unit {hidden_idx} to OUTPUT unit {output_idx} (represents char {output_char})" + '\t' +
              str(weight))
